...
from jax._src.lib import triton as jax_triton
...
def pallas_call_lowering(
    ...
    *in_nodes,
    ...
    OVERRIDE_TRITON = True
    if OVERRIDE_TRITON:
        print('overriding triton')
        #MY COMPILE START
        import triton
        ...
        res = CompResult()
        src_path = '/tmp/some.ttir'
        with open(src_path, 'w') as f:
            f.write(module_op.get_asm())
        ...
        ccinfo = triton.compile(src_path, target=GPUTarget("cuda", cc, WARP_SIZE), options=options)
        res.smem_bytes = ccinfo.metadata.shared
        res.asm = ccinfo.asm['ptx']
        res.cluster_dim_x, res.cluster_dim_y, res.cluster_dim_z = ccinfo.metadata.cluster_dims
        compilation_result = res
        #MY COMPILE END
    else:
        compilation_result = jax_triton.compile(
                lowering_platform,
                buf.getvalue(),
                arch_name,
                num_warps=num_warps,
